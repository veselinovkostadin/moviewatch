    ...
    @if(strlen($search) > 2)
        ...
        @if(count($results) > 0)
                @foreach ($results as $series)
                    <a href="{{route('tvshow.show',["show" => $series['id']])}}" class="border-b border-gray-600 text-lg py-3 px-1 flex items-center">

                            <img src="{{ config('services.tmdb.img_url') . "w45" . $series["poster_path"] }}" alt="Series poster">
                            <span class="text-xl ms-3">{{$series["original_name"]}} ({{ \Carbon\Carbon::parse($movie["release_date"] ?? "")->format('Y') }})</span>
                        </a>
